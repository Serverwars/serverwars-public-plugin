...
object CommandLobbyKick {
    ...
    fun run(ctx: CommandContext<CommandSourceStack>, kicked: Player): Int {
        try {
            val lobby = LobbyService.playerKickLobby(kicked.uniqueId)
            ctx.source.sender.sendTranslatedMessage("command.lobby.kick.success", kicked.name)
            lobby.sendMessage("command.lobby.kick.success.notify_lobby", kicked.name, lobby.participantUuids.size, lobby.lobbySettings.size)
